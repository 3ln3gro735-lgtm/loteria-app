...
def normalizar_numero(valor):
    """
    CORREGIDO: Convierte cualquier valor a entero para comparación consistente.
    Soluciona problema de 8 vs "08"
    """
    try:
        if pd.isna(valor):
            return None
...
def numero_en_lista(numero, lista):
    """
    CORREGIDO: Verifica si un número está en una lista, normalizando ambos a int.
    Soluciona el problema de 8 vs "08" en backtest
    """
    try:
    ...
    if not alertas_activas.empty:
        for _, row_alert in alertas_activas.iterrows():
            ...
            with st.container(border=True):
                st.markdown(f"**Perfil: `{perfil_name}`** | Estado: `{estado}`")
                ...
                if distribucion:
                    ...
                    st.markdown(f"📈 **Distribución:** {dist_str}")
                
                if fue_atipica:
                    st.warning(f"⚠️ **Atípico:** ({porc_ultima:.1f}% histórico)")
                if nums_alerta:
                    with st.expander(f"🔢 Ver {len(nums_alerta)} números"):
                        st.write(f"`{' '.join(nums_alerta[:20])}`")
            st.markdown("---")
    else:
...
# === BACKTEST OPTIMIZADO - 7 SORTEOS EN 30-60 SEGUNDOS ===
def ejecutar_backtest(df_full, sorteos_objetivo, debug_mode=False):
    """
    OPTIMIZADO: Backtest por NÚMERO DE SORTEOS (no días).
    7 sorteos = 30-60 segundos máximo.
    """
    if df_full.empty: return pd.DataFrame(), 0, 0, []
    
    # Obtener todos los sorteos únicos ordenados
    df_fijos = df_full[df_full['Posicion'] == 'Fijo'].copy()
    ...
    # Limitar a los últimos N sorteos
    total_sorteos_disponibles = len(df_fijos)
    sorteos_a_procesar = min(sorteos_objetivo, total_sorteos_disponibles)
    
    if sorteos_a_procesar < 1:
        return pd.DataFrame(), 0, 0, []
    
    # PRE-CALCULAR TODO UNA VEZ (CLAVE PARA VELOCIDAD)
    ...
    for i in range(sorteos_a_procesar):
        ...
        # Datos históricos hasta ANTES de este sorteo
        df_historial = df_full[df_full['Fecha'] < fecha_ref].copy()
        df_perfiles = df_cache_full[df_cache_full['Fecha'] < fecha_ref].copy() if not df_cache_full.empty else pd.DataFrame()
        
        if df_historial.empty:
            resultados.append({'Fecha': fecha_ref, 'Sorteo': tipo_sorteo, 'Real': f"{resultado_real:02d}", 'En Top30': '⚠️ Sin datos'})
        ...
        if df_perfiles.empty:
            ...
            df_stats, transizioni, ultimo_perfil = analizar_estadisticas_perfiles(df_perfiles, fecha_ref, distribuciones_cache=distribuciones_cache)
        
        prediccion = obtener_prediccion_numeros_lista(df_stats, transizioni, ultimo_perfil, df_oport_dec, df_oport_uni, df_perfiles, fecha_ref, estabilidad_digitos_global, debug_mode=False)
        
        es_acierto = numero_en_lista(resultado_real, prediccion)
        if es_acierto: aciertos += 1
        
        # LOG DETALLADO PARA NÚMERO 95 (O CUALQUIERA)
        ...
        # TIMEOUT DE SEGURIDAD (2 minutos máximo para 7 sorteos)
        if elapsed > 120:
            st.warning("⚠️ Backtest excedió 2 minutos. Deteniendo.")
            break
    
    progress_bar.empty()
    
    # GUARDAR PREDICCIONES EN CSV LOCAL PARA VERIFICACIÓN
    if debug_logs:
        try:
            df_logs = pd.DataFrame(debug_logs)
            df_logs.to_csv(RUTA_PREDICCIONES, index=False, encoding='latin-1')
        except:
            pass
...
def mostrar_tabla_personalidad_perfiles(df_historial_perfiles):
    ...
    st.markdown("Cada perfil tiene su propia 'personalidad' estadística.")
    ...
    st.caption(f"📊 {len(df_tabla)} perfiles | {datetime.now().strftime('%d/%m %H:%M')}")
...
def main():
    ...
    # === TABLA DE PERSONALIDAD (CORREGIDO: obtener_historial_perfiles_cacheado) ===
    ...
    st.markdown("---")
    st.header("🧪 Backtesting (7 Sorteos = 30-60 seg)")
    st.markdown("**Optimizado:** Backtest por número de sorteos, no días. Incluye logging detallado del 95.")
                ...
                if total > 0:
                    ...
                    if debug_logs:
                        ...
                        if logs_95:
                            ...
                            try:
                                pd.DataFrame(debug_logs).to_csv('backtest_detalle.csv', index=False, encoding='latin-1')
                                st.info(f"📁 Detalle guardado en `backtest_detalle.csv`")
                            except: pass
